    ...
    return (
        <div className="min-h-screen bg-white dark:bg-solarBlue page-pt pb-24 relative overflow-hidden">
            ...
            <div className="container-main relative z-10 max-w-5xl">
                <div className="mb-20">
                    <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 text-slate-500 hover:text-solarGreen mb-8 transition-all group">
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">
                            <FileText className="w-4 h-4 text-solarGreen" /> Final Review
                        </div>
                    </div>
                </div>

                <div className="bg-slate-50 dark:bg-white/5 backdrop-blur-3xl rounded-[60px] p-10 md:p-16 border border-slate-200 dark:border-white/10 shadow-3xl relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 mb-16">
                        ...
                        <div className="space-y-12">
                            <section>
                                <h3 className="text-[10px] font-black mb-8 flex items-center gap-4 uppercase tracking-[0.4em] text-slate-400">
                                    <div className="w-8 h-8 rounded-xl bg-solarGreen/10 text-solarGreen flex items-center justify-center">
                                ...
                                <div className="pl-12 space-y-4">
                                    <div className="flex items-start gap-4">
                                        <CheckCircle2 className="w-4 h-4 text-solarGreen mt-1" />
                                        <div>
                                            <p className="text-lg font-black tracking-tight text-slate-900 dark:text-white">{checkoutData.first_name} {checkoutData.last_name}</p>
                                        </div>
                                    </div>
                                    <div className="p-6 bg-white dark:bg-slate-900/50 rounded-3xl border border-slate-100 dark:border-white/5 shadow-inner">
                            ...
                            <section>
                                ...
                                <div className="pl-12 flex items-center gap-4">
                                    <div className="px-6 py-4 bg-solarGreen text-solarBlue rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-solarGreen/20">
                                        {paymentMethod === 'cod' ? 'Cash on Delivery' : 'Online Payment'}
                                    </div>
                                    <div className="flex items-center gap-2 text-[10px] font-black opacity-30 uppercase tracking-widest italic">
                        ...
                        {/* Order Manifest */}
                        <div>
                            <div className="bg-slate-950 text-white rounded-[40px] p-8 md:p-10 border border-white/10 shadow-3xl relative overflow-hidden group">
                                ...
                                <h3 className="text-[10px] font-black mb-10 pb-6 border-b border-white/10 uppercase tracking-[0.4em] text-white/40 flex items-center justify-between">
                                    Order Summary
                                    <span className="w-2 h-2 bg-solarGreen rounded-full shadow-[0_0_8px_rgba(100,255,153,1)]" />
                                </h3>

                                <div className="space-y-6 mb-10 pr-2 max-h-[300px] overflow-y-auto no-scrollbar">
                                    {cart.items.map((item) => (
                                        <div key={item.key} className="flex justify-between items-start group/item">
                                            <div className="space-y-1">
                                                <span className="text-[10px] font-black uppercase tracking-tight text-white block group-hover/item:text-solarGreen transition-colors">{item.name}</span>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 pt-10 border-t border-slate-100 dark:border-white/5">
                        <button
                            onClick={handleConfirmOrder}
                            disabled={isProcessing}
                            className={`group relative flex-1 flex items-center justify-between pl-10 pr-6 py-6 bg-solarGreen text-solarBlue rounded-[35px] font-black uppercase tracking-[0.3em] text-[11px] shadow-2xl shadow-solarGreen/20 transition-all overflow-hidden ${isProcessing ? 'opacity-70 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-[0.98]'}`}
                        >
                        ...
                        <button
                            onClick={() => navigate(-1)}
                            disabled={isProcessing}
                            className="px-10 py-6 bg-slate-100 dark:bg-white/5 text-slate-400 dark:text-slate-400 rounded-[35px] font-black uppercase tracking-[0.2em] text-[10px] hover:bg-slate-200 dark:hover:bg-white/10 transition-all disabled:opacity-50"
                        >
                            Cancel
                        </button>
                    </div>
